...
- 含义：与该频点的幅度平方成正比。
- STM32 显示前会执行近似 `sqrt(mag2_hi)`，再映射到 `0…255`。
- FPGA 若内部幅度平方超过16位，应使用固定移位或饱和得到16位值；
  不要每个频点使用不同的缩放系数。
...
```verilog
mag2_u16 = (mag2_shifted > 16'hffff) ? 16'hffff
                                      : mag2_shifted[15:0];
```
...
幅度和相位必须来自同一个频率索引。STM32 降采样时会在每组频点中选择
最大幅度点，并同时取该点对应的相位。
...
STM32 会把任意 N 点按比例分成64组，每组取最大幅度和对应相位，因此当前
串口屏每张图最终收到64个点。FPGA仍建议发送完整1024点，以保留降采样前的
峰值信息。
...
联调建议先发送 `N=64` 的固定斜坡/峰形数据，每500 ms一帧；STM32确认
