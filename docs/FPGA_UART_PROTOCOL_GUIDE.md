...
`N=1024` 时总长度为 `4102` 字节，在 1 Mbaud、8N1 下传输时间约
`41.02 ms`。
...
- `mag2_hi` 是协议沿用的字段名；实际发送值已经由 FPGA 完成开方，
  与该频点的幅度成正比。
- STM32 不再开方，分组求均值后直接线性映射到 `0…255`。
- FPGA 应对开方后的结果使用固定移位或饱和得到16位值；
  不要每个频点使用不同的缩放系数。
...
```verilog
amplitude_u16 = (amplitude_scaled > 16'hffff) ? 16'hffff
                                              : amplitude_scaled[15:0];
```
...
STM32 会把任意 N 点按比例分成256组，每组计算幅度和相位均值，因此当前
串口屏每张图最终收到256个点。FPGA仍发送完整1024点；正式配置下每组正好
包含连续4个频率点。
...
联调建议先发送 `N=256` 的固定斜坡/峰形数据，每500 ms一帧；STM32确认
`frame_valid_count` 持续增加后，再切换到1024点真实扫频数据。
...
```text
last_frame_bytes  = 4102
last_point_count  = 1024
frame_valid_count 持续增加
frame_error_count = 0
uart_error_count  = 0
```
