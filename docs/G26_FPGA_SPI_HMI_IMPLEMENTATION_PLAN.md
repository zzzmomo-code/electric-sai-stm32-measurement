...
当前生成状态不是最终配置：SPI1 被意外启用；SPI3 仍为 4 bit、32 MHz；
PA15 仍为 SPI3_NSS；PD1、SPI3_RX DMA 和 SPI3_TX DMA 尚未生成。下一次
写业务代码前必须完成上述 IOC 配置、保存并重新生成代码。
...
```c
uint8_t waveform_1cycle[700];
uint8_t waveform_3cycle[700];
uint8_t spectrum_display[700];
```
...
- 三周期显示对完整数据做分段重采样，输出 700 点。
- 一周期显示从完整三周期中选择一个连续完整周期，再输出 700 点。
...
- 1312 点压缩至 700 点时，每个区间取最大值，避免窄谱线被平均掉。
- 显示参数使用 FPGA 帧头中的精测结果，不从700点显示数组反推。

## 串口屏页面

串口屏使用一个页面，放置三个位置和尺寸完全重叠的 Waveform 控件：

- `s_t1`：一周期时域。
- `s_t3`：三周期时域。
- `s_spec`：频谱。

三个控件宽度按 700 个显示点设计。页面同时放置 Vpp、Vrms、基频、
分量频率/幅值和通信状态文本。
...
按键时只隐藏另外两个控件并显示目标控件。如果实测发现淘晶驰隐藏控件
不能可靠接收或保留 Waveform 数据，则使用备用策略：

```text
按键 -> 选择STM32已有缓存 -> 清空共用曲线 -> 发送700点 -> 显示
```

512000 baud、8N1下，700个单字节点的纯数据时间约 13.7 ms，加上指令和
屏幕处理时间仍远小于 2 秒。
