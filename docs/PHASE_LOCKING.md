...
最终版默认单通道模式，只使用 PA4，PA5 保持约 1.65 V 中点。双通道扩展中 PA4 固定输出
较低频分量，PA5 固定输出较高频分量。
...
这里必须区分两件事：32768 点 FFT 模块只返回频率；FFT 完成后，
`analyze_frame()` 再使用当时最新 512 点 DMA 副本的前 500 点测量初始幅值、
初始相位并判断波形类型。因此“首次判频精确”不等于“低频波形分类一定精确”。
1～4 kHz 在 500 点窗口中包含的周期数很少，三次/五次谐波比例容易受直流偏置、
起始相位和噪声影响，这是当前最终版的已知限制。
