...
**数据含义**：
- mag2_hi: 沿用协议字段名；实际为 FPGA 已开方的16位幅度响应，0~65535
...
- **降采样**：任意点数 N -> 256 点（1024点时每桶4点，幅度和有符号相位分别求均值）
- **幅度映射**：FPGA 已完成开方，STM32 将16位幅值均值直接线性映射到 0-255
- **相位映射**：(phase + 32768) / 65535 * 255 -> 0-255
- **构帧**：`cle s0.id,0` + 256 条 `add s0.id,0,val` + `cle s1.id,0` + 256 条 `add s1.id,0,val`
- 每帧最坏约 9242 字节，缓冲区预留 10240 字节；9600 baud 下发约 9.6 秒
