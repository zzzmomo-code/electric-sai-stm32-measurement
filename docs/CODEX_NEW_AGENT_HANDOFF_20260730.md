...
- FPGA负责测量和生成完整测量帧。
- STM32负责SPI协议、CRC、缓存、周期提取、频谱压缩、校准和HMI驱动。
- 串口屏只负责控件显示和发送按键命令。
...
| `measurement_conversion.c/.h` | 精确一周期/三周期提取、350点重采样、频谱压缩 |
...
- 第一份有效帧立即接受；
- 后续输入明显变化后，需要连续3帧彼此稳定才替换工作快照；
- 与当前结果接近的小抖动被忽略，减少串口屏频繁清屏闪烁。

当前旧逻辑只自动刷新数字和频谱，时域波形曾被冻结到按键触发。用户最新要求已经改为：**接受新稳定快照后，一周期和三周期都更新。**
