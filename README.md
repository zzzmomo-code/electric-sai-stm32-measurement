...
ADC DMA 写完后，CPU 读取前调用 D-Cache invalidate；CPU 写完 DAC 半区后，DMA 读取前调用 D-Cache clean。
...
所有 ADC/DAC HAL DMA 回调只设置一个对应标志；耗时处理全部在主循环完成。
...
9. 再测试三角波分类、不同幅度组合和 10～100 kHz 边界。
...
- 使用内部 HSI，绝对频率精度和温漂不如外部晶振；实板相位误差需要示波器确认。
