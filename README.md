...
- 每帧先使用实际平均值去除偏置，再计算交流真 RMS；正弦波、方波和三角波分别按 RMS 换算抗噪声 Vpp，未知波形暂用时域跨度兜底。
- 前级尚未标定时，频率、相位和波形照常分析；Vpp、DC、RMS 按 3.3 V 标称参考给出 ADC 引脚侧估算，并通过 `~` 和 `EST` 明确标记。
- 串口屏通过 `t_voltage`、`t_voltage2` 显示两路完整 8192 点分析帧的浮点平均电压；未标定估算值保留 `~` 前缀。
...
前级传递函数确认前，默认校准 `valid=0`。此时屏幕幅度显示为 `~x.xxx Vpp`，通道状态为 `EST`；该值只是按 `3.3 V / 65535` 得到的 ADC 引脚侧估算，不代表前级输入端真实电压。

每帧的直流值取校准后样本的算术平均；交流 RMS 和 FFT 都先减去该帧实际平均值，不写死 1.65 V。这样交流偏置有小幅漂移时不会直接污染幅度和频谱，直流输入本身则仍保留为 DC 测量结果。
...
以上数值仅示范接口，不是最终硬件参数。最终参数必须根据前级比例和两点或多点实测标定计算；标定有效后屏幕自动改为无 `~` 的数值和 `LIVE` 状态。
