...
1. 单信号默认同时收集 32768 点原始记录和 32 倍抽取记录：原始记录负责
   1.2～400 kHz，抽取记录以 78.125 kSPS 覆盖 40 Hz～1.2 kHz；两路都经过
   直流去除、Hann 窗、FFT 峰值插值和前后半段相位斜率细化。
2. 初值直接转换成 Q32 NCO 相位步进，不重新取整成整数 Hz。
3. 1 kHz 以上每 512 个 ADC 样点做一次带直流项的最小二乘测相；40 Hz～1 kHz
   使用带迟滞确认和线性插值的上升中点过零，正弦、三角和方波共用相位 0。
   双信号仍在同一帧联合求解四个正余弦系数，原通路不变。
...
`SIGSEP_MODE_SINGLE`：PC0 输入一路干净信号，PA4 重建并锁相，PA5 保持中点。
双混合输入代码仍完整保留，需要时再把这一行改回 `SIGSEP_MODE_DUAL_MIXED`。
...
当前源码默认使用 `SIGSEP_MODE_SINGLE`。在该模式下只使用 PA4，PA5 保持 DAC
中点；`SIGSEP_COMMON_SOURCE_LOCK` 只在双混合模式下有意义。
...
| `SIGSEP_FREQ_MODE_PRECISE_FFT` | 默认高精度首次判频方案 | 原始/32 倍抽取两条 32768 点记录、Hann 窗 FFT、三点对数谱峰插值、相位斜率细化和长记录谐波分类 |

当前源码默认使用 `SIGSEP_FREQ_MODE_PRECISE_FFT`。原始记录为 13.1072 ms，
频点间隔约 76.2939 Hz；32 倍抽取记录为 419.4304 ms，频点间隔约
2.3842 Hz。为了优先保证第一次判频和波形分类正确，单信号模式会等待低频记录
完成后再比较低/高频候选，所以首次输出约需 0.42 s。单信号默认搜索范围为
40 Hz～400 kHz，范围宏为
`SIGSEP_PRECISE_FREQ_MIN_HZ` 和 `SIGSEP_PRECISE_FREQ_MAX_HZ`。
双混合模式保持原来的 1～250 kHz 高精度搜索边界，不随本次单信号扩展改变。
如果高精度模式实板测试出现问题，
...
- `frequency=precise_fft, range=40..400000Hz, acquire=419ms`：默认单信号高精度方案。
...
5. 快速连续频率模式先用相同的 19 点粗搜索定位主峰，同时收集 8 个完整 DMA 半区；
   ...
   主峰附近的谱泄漏，再寻找第二分量。
6. 单信号高精度模式并行生成两条 32768 点记录：原始 2.5 MSPS 记录负责
   1.2～400 kHz，32 点盒式平均抽取后的 78.125 kSPS 记录负责 40 Hz～1.2 kHz。
   两条路径复用同一组 FFT 工作区，并比较基波幅值，避免把低频方波的高次谐波
   误当成高频基波。双信号模式仍使用原始记录和 4 kHz 主峰保护区。
7. 单信号的基波、三次和五次谐波全部在同一段长记录上用 Hann 加权相关测量：
   正弦的奇次谐波接近 0，三角波理论 H3/H1=1/9，方波理论 H3/H1=1/3。
   不再用最后 500 点分类，因此 1～4 kHz 不足一周期造成的虚假谐波已消除。
8. 插值得到的毫赫兹初值直接换算为 Q32 NCO 步进；之后仍使用原 Q32 NCO 和 PI
   型数字 PLL 跟踪输入相位及频率误差，锁相环参数没有因判频升级而改变。HSI 场景的软件
   捕获范围设为约 ±2%。
9. 1 kHz 以上的单信号继续用 2×2 中心化最小二乘测幅/测相；40 Hz～1 kHz
   使用施密特迟滞确认、中心穿越线性插值和周期平滑更新 NCO。超过约 2.5 个预计
   周期没有有效过零会自动回到识别状态。双信号继续用 4×4 联合最小二乘。
   单信号只更新通道 0 并输出 PA4，PA5 始终回填中点值。
...
├─ frequency_estimator.c          原始/抽取长记录、FFT、幅相和谐波分析
├─ uart_debug.h
...
tests/
└─ frequency_estimator_host_test.c 三类波形 40 Hz～400 kHz 主机回归
...
- 实际 `frequency_estimator.c` 主机测试覆盖正弦、三角、方波三类，以及
  40、50、100、500、999、1000、1100、1200、1234.567、2000、3000、
  4000、5000、10000、250000、400000 Hz；加入 ADC 量化和随机噪声后，
  全部用例的频率路径、波形类型和频率误差判据均通过。
...
1. 确认 `SIGSEP_OPERATION_MODE` 为 `SIGSEP_MODE_SINGLE`，Clean/Build 后重新下载。
2. PC0 先输入 1 kHz、2 kHz、3 kHz、4 kHz 正弦，逐项确认串口均报告 `sin`。
3. 重复输入同频率方波和三角波，确认分别报告 `square`、`tri`，PA4 输出同类波形。
4. 依次测试 40、50、100、500、999、1000、1200、1234.567、10 kHz；
   每次改变输入频率后发送串口 `r`/`R` 或复位，使程序重新完成约 0.42 s 首次识别。
5. 用示波器同时观察 PC0 和 PA4。40 Hz～1 kHz 至少连续观察 10 个周期，
   1 kHz 以上连续观察 5 s，确认相位差不持续单向漂移。
6. 250～400 kHz 只作为实用候选上限测试：重点观察实际模拟带宽、阶梯失真和
   波形分类，不把软件合成测试当作实板质量证明。
...
- 单信号高精度模式的软件搜索范围为 40 Hz～400 kHz；40 Hz 下限和 400 kHz
  上限、1.2 kHz 路径切换点仍需逐点实板确认。2.5 MSPS 的理论奈奎斯特上限为
  1.25 MHz，但三类波形统一分类还要求三次谐波低于奈奎斯特，因此理论边界约为
  416.7 kHz。400 kHz 时 DAC 仅有 6.25 点/周期，方波/三角波会明显阶梯化。
...
- 两条 32768 点记录和复用的 FFT 工作区约占 384 KiB；当前完整 ELF 的
  `.bss` 约 401 KiB，仍位于 D1 SRAM。识别期间 DAC 暂时保持中点；FFT 只在
  启动或收到 `r` 重新识别时运行，不进入锁定后的实时路径。
- 单信号模式只使用 PA4 输出；PA5 保持中点不是故障。
- 1 kHz 以上输入幅度低于有效阈值后，PLL 会冻结相位和积分状态；改变频率档位后
  发送串口字符 `r`/`R`、复位，或调用 `signal_separation_restart_identify()`。
  40 Hz～1 kHz 低频路径连续约 2.5 个周期没有确认过零时会自动重新识别。
- 方波/三角波判别使用完整长记录的三次、五次谐波比例；1～4 kHz 的合成回归已
  通过，但阈值仍会受函数发生器带宽、前端失真和 ADC 噪声影响，需要实板标定。
