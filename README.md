...
STM32 可以预装曲线、接收两个按键命令并切换一周期/三周期可见性；频谱在两种时域
模式下均保持独立显示。
...
| 题目要求 | 当前实现 | 验证状态 |
|---|---|---|
| 按键显示 1 个或 3 个完整周期 | 从 FPGA 捕获的 3 周期数据生成 350 点一周期/三周期缓存，按键只切换左侧控件 | 自检链路通过，真实周期数待 FPGA 数据验证 |
...
| 每项上电后 2 秒内显示 | SPI 最大帧纯传输约 4.1 ms；三图后台预装后按键只发 `vis` | 理论满足，整机上电计时待测 |
