...
5. 上电首帧立即显示；按启动键立即锁存最新结果，随后最多五帧小幅平均，周期键只切换波形；
...
- 系统时钟：480 MHz；
- FPGA SPI：当前测试档为 20 MHz、Mode 0、8 bit、MSB first；
- 串口屏：512000 baud、8N1。
...
- PLL2P 80 MHz，Prescaler 4，当前 SCK 20 MHz；
...
上电后第一份有效快照立即显示参数、频谱和默认的一周期波形。此后 FPGA 帧仍在后台持续接收，
但屏幕保持冻结。按 `b_t5` 启动后立即锁存后台最新完整快照并更新全部显示；
启动帧计作第 1 帧，随后只对小变化标量最多累计五帧平均。按一周期或三周期只切换
已装载的两个重叠波形控件，不重新采样或计算。
...
发送，最终横轴为左侧低频、右侧高频；左右各固定保留8个0%基线点，极低频和极高频
谱线不会贴住边框。
...
- `Core/User/hmi_task2.c/.h`：USART1 DMA、启动锁存、五帧小幅微调、周期按键和冻结显示；
...
上电首帧立即显示，之后只有按启动键才锁存最新完整结果并更新屏幕。启动后最多五帧
小变化标量参与平均，随后冻结；周期按键只切换已经装载的波形。
若输入改变后稳定结果仍不更新，先比较
...
必然显示为相同高度。有效横坐标限制在第8～341点，最外侧各8点保持0%基线。
原始频谱数组仍接收并保留饱和诊断，但不参与串口屏绘图。

#### 第六步：启动锁存与五帧微调

`hmi_task2_refresh_work_snapshot()` 始终可以读取后台最新换算快照，但屏幕工作快照
只在上电首帧或启动键事件时替换。调度器每次只启动一个 UART TX DMA 动作：

```text
上电收到第一份有效帧：立即锁存并发送参数、一周期、三周期和频谱
→ 默认一周期控件在前，三周期控件已装载在后
→ 未按启动键：FPGA继续采集，但屏幕结果保持冻结
→ 按启动键：立即锁存后台最新完整帧并重画三条曲线
→ 启动帧计作第1帧，后续小变化帧加入平均，最多累计5帧
→ 只重发平均后的参数文字，不跨帧平均或反复重画曲线
→ 遇到大变化或800 ms到期：保留此前最新平均并冻结到下一次启动
```

三组350点数据始终缓存在STM32。每次完整锁存时，一周期、三周期和频谱都会装载；
两个时域控件完全重叠，周期键只调整前景层级。屏幕不按每个FPGA快照重画，从根源
上避免一条曲线尚未发送完就又被 `cle` 清空造成的闪烁。
...
```text
CMD=04：锁存后台最新完整快照并开启五帧小幅微调
CMD=01：把已装载的一周期控件切到前景
CMD=02：把已装载的三周期控件切到前景
CMD=03：可选，手动重画一次频谱
CMD=10：预留模式键，本版忽略
```
...
因此周期键不清空、不重画、不触发重新采样、重新FFT或要求FPGA重测；启动键也只
锁存STM32后台已经准备好的最新快照，不向FPGA发送重测命令。
...
- `stable_candidate_count` 在启动微调过程中应由1增加到5；
...
- 串口屏 USART1 收发、旧版按键命令和曲线显示已有实板证据；本次“启动立即锁存、
  五帧小幅平均、频谱冻结、周期控件切换”新逻辑仍待重新烧录验证；
- 曲线纵坐标已限制在 210 像素控件的安全范围内，当前映射范围为 8～201，避免
  淘晶驰单字节点超过控件高度后产生削顶；
- 已加入独立标量校准层和 `A5 20 5A` 切换接口；当前系数为 `y=x`，正式系数仍需
  根据标准仪器打表数据拟合并实板验收；
- FPGA SPI 正式帧已经能够驱动参数显示；当前 20 MHz 测试速率下的长期稳定性、测量精度和
  整机 2 秒指标仍待新固件实板验证；
...
| 每项上电后 2 秒内显示 | 当前20 MHz SPI最大帧约4.1 ms；上电首帧立即显示；启动微调最多5帧 | 理论满足，整机上电计时待测 |
