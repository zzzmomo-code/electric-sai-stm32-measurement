...
屏幕 5 V 供电能力、PA9 到屏幕 RX 的连线、实际波特率和约 10 Hz 刷新均为待硬件验证。
...
- `Core/User/hmi_tjc.h`、`Core/User/hmi_tjc.c`：淘晶驰文本指令格式化和 USART1 TX DMA 刷新；未生成 USART1 时保持空闲。
...
4. `hmi_tjc_process()` 在 ADS8688 处理后读取测量结果快照；USART1 已由 CubeMX 生成时才通过 TX DMA 向串口屏发送。
...
- 本分支未执行远程 push。
- USART1 尚未通过当前 `.ioc` 生成；在 CubeMX 完成 PA9/PA10、TX DMA 和中断配置前，HMI 模块不会访问 UART。这一配置及实屏刷新仍为待硬件验证。
