...
当前 SPI2 SCLK 为 16.125 MHz，单帧 32 个 SCLK，并保留 9 周期帧间空闲时间，理论总吞吐约 393 kSPS。默认自动扫描仅启用 AIN0 与 AIN1，因此每路理论采样率约为 196 kSPS；AIN0 为幅度、频率和波形类型的主信号，AIN1 用于相对 AIN0 的相位差。若实板验证发现 NSS 高电平时间、DMA 中断负载或信号完整性不足，建议先回退到 200 kSPS 级别验证稳定性，再提高时钟。
...
- 当前默认使用内部 4.096 V 基准，AIN0 与 AIN1 使用 ±10.24 V 量程；外部基准、其他量程或重新启用其他通道需要另行确认硬件后再修改。
- 本分支未执行远程 push。
